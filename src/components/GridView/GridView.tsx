...
export const GridView = (props: Props) => {
  ...
  return (
    <div className='justify-center grid grid-cols-[repeat(auto-fill,150px)] md:grid-cols-[repeat(auto-fill,216px)] gap-4 overflow-y-auto max-h-screen-82 md:max-h-screen-80 px-4'>
      {devices.map((device) => {
        return (
          <Link to={`device?id=${device.id}`} state={device} key={device.id} className='transition ease-in-out duration-300 hover:border-web-unifi-color-ublue-06 grid grid-cols-1 grid-rows-[100px_1fr_1fr] border-web-unifi-color-neutral-3 border rounded-lg shadow-sm'>
            ...
            <span className='p-1 m-1 rounded bg-web-unifi-color-neutral-0 text-xs text-web-unifi-color-ublue-06 row-start-1 col-start-1 justify-self-end self-start'>{device.line.name}</span>
