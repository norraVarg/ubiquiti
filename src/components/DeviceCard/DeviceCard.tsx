...
export const DeviceCard = (props: Props) => {
  ...
  return (
    <div role='device-card' className='transition ease-in-out duration-300 hover:border-web-unifi-color-ublue-06 grid grid-cols-1 grid-rows-[100px_1fr_1fr] border-web-unifi-color-neutral-3 border rounded-lg shadow-sm'>
      ...
      <span className='flex items-end px-2 py-1 text-opacity-85 text-web-unifi-text-0 leading-[20px]'>{device.product.name}</span>
      <span className='flex p-2 text-sm text-opacity-45 text-web-unifi-text-0 leading-[16px]'>{device.shortnames.join(', ')}</span>
    </div>
