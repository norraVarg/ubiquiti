  ...
  return (
    <>
      ...
      <div className='overflow-y-auto max-h-screen-70'>
        {devices.map((device) => {
          return (
            <Link to={`device?id=${device.id}`} key={device.id} className='transition ease-in-out duration-300 hover:bg-web-unifi-color-ublue-06 hover:bg-opacity-5 grid grid-cols-[40px_1fr_2fr] text-web-unifi-text-1 text-opacity-60 items-center h-9 border-b border-web-unifi-color-neutral-3'>
